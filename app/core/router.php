...
class Router {
	 ...
	 * @param   array @params
	 */
	public static function __callstatic($method, $params){//echo $method." - "; print_r($params);echo "<br/>";
	...
	public static function anyone($method, $params){
		$params = array(0 => $method, 1 => $params);
		$method="any";
		//echo $method." - "; print_r($params);echo "<br/>";
		$uri = dirname($_SERVER['PHP_SELF']).'/'.$params[0];
		$callback = $params[1];
	...
	public static function invokeObject($callback,$matched = null,$msg = null){
		...
		//echo "app".str_replace("\\","/",$segments[0]).".php";
		require "app".str_replace("\\","/",$segments[0]).".php";
		$temp = explode("\\", $segments[0]);
		$index = sizeof($temp)-1;				
		$temp = $temp[$index];
		$controller = new $temp($msg);
	...
	public static function dispatch(){
		...
		$searches = array_keys(Router::$patterns);
		$replaces = array_values(Router::$patterns);
		...
		if (!$found_route) {
			if (!self::$error_callback) {
				self::$error_callback = Router::handleError();
			} 
